...
function ddb_pdc_result($ddbid)
{
	...
	switch ($pdcresult->code) {
		case "500":
			drupal_get_messages();
			drupal_set_message(t('Unter der angegebenen ID wurden keine Ergebnisse gefunden. Bitte überprüfen Sie Ihre Eingabe!'), 'error');
		...
		case "200":
			...
			for ($i = 0; $i < count($json_searchresults); $i++) {
				if ($json_searchresults[$i]->id == $ddbid) {
			...
			$output .= '</div>';
			$output .= '<div class="item-license"><img src="' . $imagespath . 'icons/' . $publicdomain . '.png" title="' . $resultImgAlt . '"/></div>';
			$output .= '</div>';
			...
			foreach ($json_pdcresult->trace as $question) {
				...
				$output .= '<div class="pdc-question"><img src="' . $imagespath . 'icons/' . 'question_' . $questionResult . '.png" alt="" />' . $question->question . '</div>';
				$output .= '<div class="pdc-answer"><img src="' . $imagespath . 'icons/' . 'answer_' . $questionResult . '.png" alt="" />' . $questionResultTrace . '<span class="notes">' . $question->note . '</span></div>';
				$output .= '</div>';
